...
module For_diff = struct
  module Diff = struct
    ...
    let yojson_of_t { in_source; in_build } =
      `Assoc [ "in_build", `String in_build; "in_source", `String in_source ]
    ;;
  end
  ...
  Drpc.Registry.Poll
    ...
    (struct
      ...
      let stat s =
        let+ () = Fiber.return () in
        match
          if Sys.win32 && is_414 && not (Sys.file_exists s)
          then
...
module Instance : sig
    ...
    let* () =
      Fiber.all_concurrently_unit
        ...
        ; (let init =
           ...
           in
           let* () =
             let message = sprintf "client %d: connecting..." (Id.to_int running.id) in
             config.log ~type_:Info ~message
           in
           ...
           Client.connect chan init ~f:(fun client ->
             ...
             let* () =
               let message =
                 sprintf "client %d: connected to dune at %s" (Id.to_int running.id) where
               in
               config.log ~type_:Info ~message
             in
  ...
  let format_dune_file t doc =
    ...
    | Running { client = Some client; _ } ->
      let* req = Client.Versioned.prepare_request client Drpc.Request.format_dune_file in
      let req =
        match req with
...
let create
  ...
  let config =
    let include_promotions =
      ...
      | _ -> false
    in
    { document_store; diagnostics; progress; position_encoding; include_promotions; log }
  in
...
module Promote = struct
  module Input = struct
    ...
    let yojson_of_t { dune; in_source } : Json.t =
      `Assoc [ "dune", `String dune; "in_source", Json.Conv.yojson_of_string in_source ]
    ;;

    let t_of_yojson = function
      | `Assoc fields ->
        let dune = Json.field_exn fields "dune" Json.Conv.string_of_yojson in
        let in_source = Json.field_exn fields "in_source" Json.Conv.string_of_yojson in
        { dune; in_source }
      | json -> Json.error "invalid promote data" json
    ;;

    let create (dune : Registry.Dune.t) (promotion : Drpc.Diagnostic.Promotion.t) =
        ...
        (match String.Map.find active.instances promote.dune with
         ...
         | Some instance ->
           (match Instance.client instance with
            ...
            | Some client ->
              let+ res =
                let* req = Client.Versioned.prepare_request client Drpc.Request.promote in
                let req =
                  match req with
                  | Error _ -> assert false
                  | Ok req -> req
...
let code_actions (t : t) (uri : Uri.t) =
  ...
  | Active active ->
    ...
    String.Map.fold active.instances ~init:[] ~f:(fun dune acc ->
      ...
      | Some promotion ->
        let command =
          let promote = Promote.Input.create (Instance.source dune) promotion in
          Command.create
            ~title:"Promote"
        ...
        in
        let action =
          CodeAction.create ~title:"Promote" ~kind:CodeActionKind.QuickFix ~command ()
        in
        action :: acc)
;;
