...
(** This module is a collection of client-specific functionality (client = editor) *)
...
module Vscode : sig
  ...
  module Commands : sig
    (** [editor.action.triggerSuggest] is a vscode-specific command, which triggers the
        completion request on all completion providers *)
...
module Custom_commands : sig
  (** Request client cursor to jump to the next hole.
      ...
      See the documentation for this command in [vscode-ocaml-platform] for details.

      @param in_range
        to pick a hole only in a given range; if omitted, the whole document is used
      @param notify_if_no_hole
        specifies whether we want the client to show the user a message if there is no
        hole to jump to *)
