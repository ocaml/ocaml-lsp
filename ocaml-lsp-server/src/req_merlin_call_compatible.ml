  ...
  | `Merlin merlin ->
    ...
    let config, args =
      Mconfig.parse_arguments
        ...
        cmd_args
    in
    Document.Merlin.with_configurable_pipeline_exn ~log_info ~config merlin (f args)
;;
...
let on_request ~(log_info : Lsp_timing_logger.t) ~params state =
  Fiber.of_thunk (fun () ->
    ...
    in
    let log_info = { log_info with action = log_info.action ^ "-" ^ command } in
