...
let%expect_test "a helper that cannot be started is unavailable" =
  let dir = Test.temp_dir "ocamllsp-on-type-formatting-unavailable-" in
  let bin_dir = Filename.concat dir "bin" in
  Unix.mkdir bin_dir 0o700;
  let helper = Filename.concat bin_dir "ocp-indent-rpc" in
  Test.write_file helper "#!/bin/sh\nexit 0\n";
  Unix.chmod helper 0o600;
  ...
  [%expect {| on-type formatting unavailable |}]
;;
