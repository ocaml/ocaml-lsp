...
`,
  ...
  it("refactor-open unqualify stdlib module", async () => {
    let specificCodeActions = await testRefactorOpen({
      documentText: outdent`
      open Unix

      let times = Unix.times ()
      let f x = x.Unix.tms_stime
      `,
      queryStartPos: Types.Position.create(1, 6),
      queryEndPos: Types.Position.create(1, 6),
      codeActionTitle: "Remove module name from identifiers",
    });
    let fileUri = "file:///test.ml";
  ...
  it("refactor-open qualify stdlib module", async () => {
    let specificCodeActions = await testRefactorOpen({
      documentText: outdent`
      open Unix
