    ...
    let rec loop d acc =
      ...
      | `Uchar _ | `Malformed _ ->
          ...
          if new_line > old_line then
            let line_ofs = Uutf.decoder_byte_count d / 2 in (* UTF16 encodes on 2 bytes *)
            loop d (line_ofs::acc)
          else
            loop d acc
      | `End ->
...
let utf16_offsetAt (text: string) ({ line; character }: Position.t) =
  if line < 0 then 0
    ...
    if line >= al - 1 then lofs.(al - 1) else
    let this_lofs = lofs.(line) in
    let next_line_offset = lofs.(line + 1) in
    max (
      min (this_lofs + character) (next_line_offset)
    ) this_lofs

let byte_offsetAt t pos =
  2 * utf16_offsetAt t pos

let utf16_range_change (text_utf8: string) ({ start; end_ }: Range.t) change_utf16 =
