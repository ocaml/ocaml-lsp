...
type t =
  ...
  | Variable of
      var * [ `Placeholder of t | `Transform of variable_transform | `None ]
  | Text of string
  | Concat of t * t
...
module Grammar = struct
  ...
  let choice ?index values =
    if List.is_empty values then
      raise (Invalid_argument "choice must have non empty values")
    else
  ...
  let rec go ctx = function
    ...
    | Variable (var, opt) -> (
      ...
      | `Transform t ->
        ...
        ctx )
  in
  go (1, IMap.empty) snippet |> ignore
...
let pp_t (chan : Format.formatter) (t : t) : unit =
