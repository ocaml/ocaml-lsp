...
and process_watcher =
  ...
  ; something_is_died_unix : Condition.t
...
module Process_watcher : sig
  ...
  let wait_unix t =
    while not t.table_dirty_unix do
      Condition.wait t.something_is_died_unix t.mutex
    done;
  ...
  let set_sigchld_handler_unix t =
    let sig_handler _sig =
      ...
      Condition.signal t.something_is_died_unix
    in
  ...
  let init process_scheduler =
    let t =
      ...
      ; something_is_died_unix = Condition.create ()
