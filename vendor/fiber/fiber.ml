 ...
 *
 *   module Dlist = struct
 *     type 'a t = 'a t' option ref
 *     and 'a t' = { head : 'a t ; tail : 'a t }
 *     and 'a node =
 *       { left : 'a t
 *       ; right : 'a t
 *       ; data : 'a
 ...
 *
 *     let singleton data =
 *       let node = { left = empty (); right = empty (); data } in
 *       ref (Some node)
 *
 *     let push t data =
 *       match !t with
 *       | None -> t := !(singleton data)
 *       | Some t' ->
 *         let new_tail = { left = t'.tail ; right = empty () ; data } in
 *         t'.tail.right <- new_tail;
 *         t := { t' with tail = new_tail }
 *
 *     let unshift t =
 *       match !t with
 *       | None -> None
 *       | Some t' ->
 *         let data = t'.head.data in
 *         t := t'.head.right;
 *         data
 *   end
 *   type t =
 *     { mutex : Mutex.t
 *     ; pending : (unit -> 'a fiber) Dlist.t
 *     ; max_threads : int
 *     ; cond : Condition.t
 *     ; mutable available : Thread.t list
 *     }
 *   and 'a thread =
 *     { self : Thread.t
 *     ; ivar : 'a Ivar.t
 ...
 *     }
 *   and 'a job =
 *     | Pending of (unit -> 'a t) Dlist.Node.t
 *     | In_progress of 'a t Ivar.t
 * end *)
